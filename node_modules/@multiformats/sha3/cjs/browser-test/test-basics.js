...
const fixtures = [
  ...
  [
    'shake-128',
    'foo',
    '1820f84e95cb5fbd2038863ab27d3cdeac295ad2d4ab96ad1f4b070c0bf36078ef08'
  ],
  [
    'shake-256',
    'foo',
...
describe('Hashers', () => {
  ...
  const codecs = table_csv.split('\n').filter(Boolean).map(l => l.split(',').map(e => e.trim()).map(e => e.startsWith('0x') ? parseInt(e, 16) : e));
  for (const [name, , code] of codecs) {
    const exportName = name.replace('-', '');
    it(name, async () => {
      const hasher = index[exportName];
